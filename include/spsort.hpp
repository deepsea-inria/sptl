...
Chunkedseq mergesort(Chunkedseq& xs, const Compare& compare) {
  ...
  level4::reduce(in, out, id, result, convert_reduce, seq_convert_reduce);
  return result;
}
...
template <class Item, class Compare>
void mergesort(Item* xs, Item* tmp, size_t lo, size_t hi, const Compare& compare) {
  using input_type = level4::tabulate_input;
  auto merge_fct = [&] (size_t lo, size_t mid, size_t hi) {
...
void merge(Input_iter first1,
           ...
           const Compare& compare) {
  size_t lo_xs = 0;
  size_t hi_xs = last1 - first1;
  size_t lo_ys = 0;
  size_t hi_ys = last2 - first2;
  size_t lo_tmp = 0;
