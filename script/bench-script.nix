...
let autotune = import sources.pbenchOcamlSrcs.pbenchCustom {
                      benchSrc = sources.benchOcamlSrc;
                      bench = "${autotuneDune}/bin/autotune"; };
in

stdenv.mkDerivation rec {
  name = "bench-script";

  src = "${sources.sptlSrc}/script/dummy";
  ...
  installPhase = ''
    ...
    cp ${autotune}/bench $out/autotune
    wrapProgram $out/autotune \
      --prefix PATH ":" $out/ \
